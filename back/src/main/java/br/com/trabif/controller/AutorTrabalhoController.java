...
public class AutorTrabalhoController {
	...
	@CrossOrigin("http://localhost:4200")
	public ResponseEntity<AutorTrabalhoDTO> addAutorTrabalho(@ModelAttribute AutorTrabalhoConversorDTO autorTrabalho)
			throws URISyntaxException {
		try {
			AutorTrabalho autorTrabalho2 = new AutorTrabalho();
			autorTrabalho2 = autorTrabalho.convertObj(); 
			AutorTrabalho novoAutorTrabalho = autorTrabalhoService.save(autorTrabalho2);

			return ResponseEntity.created(new URI("/api/autorTrabalho" + novoAutorTrabalho.getId()))
					.body(new AutorTrabalhoDTO().converter(novoAutorTrabalho));
